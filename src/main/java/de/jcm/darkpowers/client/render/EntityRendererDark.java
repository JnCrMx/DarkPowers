...
public class EntityRendererDark extends EntityRenderer
{
	...
	public void updateCameraAndRender(float p_78480_1_)
	{
		...
		ClientProxy proxy = (ClientProxy) DarkPowers.commonProxy;
		if(proxy.blackArrowCamera != null)
		{
			if(proxy.blackArrowCamera.isDead)
			{
				if(proxy.blackArrowCamera == Minecraft.getMinecraft().renderViewEntity)
				{
			...
			else
			{
				if(proxy.blackArrowCamera != Minecraft.getMinecraft().renderViewEntity)
				{
					Minecraft.getMinecraft().renderViewEntity = proxy.blackArrowCamera;
				}
			}
		}
